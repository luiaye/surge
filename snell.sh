...
# 开启 TCP Fast Open + 网络优化（写 sysctl 专用文件）
enableTCPFastOpen() {
...
downloadSnell(){
    ...
    # 注意：精简版已移除 --no-check-certificate（如你强制需要，可自行加回）
    wget -N "${snell_url}"
...
# 设置协议版本（仅 v4/v5）
setVer(){
    echo -e "配置 Snell Server 协议版本${Yellow_font_prefix}[4-5]${Font_color_suffix}
==================================
${Green_font_prefix} 1.${Font_color_suffix} v4 ${Green_font_prefix} 2.${Font_color_suffix} v5
=================================="
...
${Green_font_prefix} 1.${Font_color_suffix} v4  ${Green_font_prefix} 2.${Font_color_suffix} v5
=================================="
    read -e -p "(默认：1.v4)：" choose_ver
    [[ -z "${choose_ver}" ]] && choose_ver="4"
...
==============================
 ...
 ${Green_font_prefix}8.${Font_color_suffix} 配置 Snell Server 协议版本(v4/v5)
==============================
 ${Green_font_prefix}9.${Font_color_suffix} 修改 全部配置" && echo
    ...
    case "$modify" in
        ...
        8) setVer ;;
        9)
            setPort; setPSK; setObfs; setIpv6; setTFO; setDNS; setVer
            ;;
